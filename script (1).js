...
let finalTranscriptAccumulator = ''; // Stores the finalized text
...
if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    // IMPORTANT: 'continuous = false' is the absolute best way to prevent the repeating bug on Windows Chrome.
    // It captures one phrase, stops, and we auto-restart it, preventing internal array duplication bugs.
    ...
    recognition.onresult = (event) => {
        ...
        if (finalPhrase) {
            ...
            voiceResult.value = finalTranscriptAccumulator + interimTranscript;
            voiceResult.scrollTop = voiceResult.scrollHeight;
        }
    };

    recognition.onerror = (event) => {
        // Ignore "no-speech" errors, they happen naturally when continuous = false and user is quiet
    ...
    recognition.onend = () => {
        if (isRecording) {
            // Automatically restart immediately to simulate continuous listening without the bugs
            try {
                recognition.start();
            } catch (e) {
                console.error(e);
    ...
    startBtn.addEventListener('click', () => {
        if (isRecording) {
            ...
            // Sync accumulator in case user manually typed
            finalTranscriptAccumulator = voiceResult.value;
            // Add space if there is already text
            if(finalTranscriptAccumulator && !finalTranscriptAccumulator.endsWith(' ')) {
    ...
    // Sync accumulator if user types manually
    voiceResult.addEventListener('input', () => {
        finalTranscriptAccumulator = voiceResult.value;
        updateStats('voice-result', 'voice-stats');
    });
...
clearVoiceBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
        voiceResult.value = '';
        finalTranscriptAccumulator = ''; // Reset accumulator too!
